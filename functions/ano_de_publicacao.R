...
ano_de_publicacao <- fromJSON("http://172.16.17.42:8080/solr/biblio/select?facet.field=publishDate&facet.field=institution&facet.field=format&facet=on&fl=publishDate&indent=on&q=*:*&fq=cannabis&wt=json")

#View(ano_de_publicacao)

x <- ano_de_publicacao$facet_counts$facet_fields$publishDate
#y <- ano_de_publicacao$facet_counts$facet_fields$format
...
# Filtro
ano_de_publicacao <- filter(x, publishDate < '2022-01-01' & publishDate > '1972-01-01')
